                ...
                <form  id="forgotpassword" action="{{ route('password.email') }}" method="post">
                    ...
                    <div class="row">
                        <div class="col-12 mb-3">
                            ...
                            placeholder="{{trans('label.email_address')}}" name="email" value="{{ old('email') }}" required autofocus>
                            @if ($errors->has('email'))
                            ...
                            <div class="row col-sm-12">
                                <button class="btn bg-orange orange-btn text-white font-14 rounded-1 px-5 mt-3 font-GilroySemiBold"> {{trans('label.send_password_reset_link')}}</button>
                            </div>
                        </div>  
                    </div>
                </form>
