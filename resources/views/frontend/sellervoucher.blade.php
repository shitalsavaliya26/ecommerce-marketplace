@foreach($seller->vouchers as $voucher)
    <div class="col-12 mt-2">
        <input type="hidden" name="voucher" id="voucher{{$voucher->id}}" value="{{$voucher->id}}">
        <input type="hidden" name="sellerId" id="sellerId{{$seller->id}}" value="{{$seller->id}}">
        <div class="d-flex flex-column flex-md-row align-items-md-center border-voucher shadow-sm">
            ...
            <div class="d-flex justify-content-between align-items-center p-3 w-md-75">
                ...
                <div>
                    <input class="cursor-pointer shadow-none" type="radio" name="voucher" value="{{$voucher->id}}" @if($voucherID == $voucher->id || $sellervoucher == $voucher->id) checked @endif>
                </div>
            </div>
        </div>
    </div>
