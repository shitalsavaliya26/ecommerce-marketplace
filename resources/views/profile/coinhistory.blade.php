                ...
                <div class="col-12 px-0">
                    <nav class="profile-tabs profile-tabs-w-25">
                        <div class="nav nav-tabs border-0 flex-column flex-md-row flex-lg-column flex-xl-row" id="nav-tab" role="tablist">
                            ...
                            aria-selected="true">{{trans('label.coin_history')}}</a>
                        </div>
                    </nav>
                </div>
            </div>
                        ...
                        <div class="tab-pane fade show active" id="coinHistory" role="tabpanel" aria-labelledby="coinHistory-tab">
                            <div class="row br-15 bg-white pt-4 pb-5 shadow overflow-hidden">
                                <div class="col-12 mt-2  table-responsive" id="coinsHistory">
                                    <table class="table history-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col">{{trans('label.earned_coins')}}</th>
                                                <th scope="col">{{trans('label.used_coins')}}</th>
                                                <th scope="col">{{trans('label.ordered_on_small')}}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @if(count($coinsHistory) > 0)
                                            @foreach($coinsHistory as $coins)
                                            <tr>
                                                <td><a href="{{ route('user.orders.view',$coins->order_id) }}" class="cursor-pointer text-light-blue ml-2 font-GilroySemiBold">{{ $coins->order_id}}</a></td>
                                                <td>{{ number_format($coins->earned_coins,2) }}</td>
                                                <td> @if($coins->used_coins > 0) - @endif {{ number_format($coins->used_coins,2) }}</td>
                                                <td>{{ date('d/m/Y',strtotime($coins->created_at)) }}</td>
                                            </tr>
                                            @endforeach
                                            @else
                                            <tr>
                                                <td colspan="7">
                                                    {{trans('label.no_coins_history_found')}}.
                                                </td>
                                            </tr>
                                            @endif
                                        </tbody>
                                    ...
                                    @if(count($coinsHistory) > 0)
                                    <div class="col-12">
                                        {{ $coinsHistory->render('vendor.default_paginate') }}
                                    </div>
                                    @endif
                                </div>
                            </div>
